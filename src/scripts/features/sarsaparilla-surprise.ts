...
function registerSocketHandler() {
  ...
  if (state.handler) {
    game.socket.off(moduleSocket, state.handler);
  }
  ...
  game.socket.on(moduleSocket, state.handler);
}

async function broadcastTwang(card: DrawnCard) {
  const local = enqueueAnimation(() => playTwangAnimation(card));
  game.socket.emit(moduleSocket, { type: "twang", userId: game.user.id, card });
  await local;
}
...
async function broadcastStandReveal(cards: DrawnCard[], labelText: string, labelColor: string) {
  ...
  game.socket.emit(moduleSocket, { type: "stand", userId: game.user.id, cards, labelText, labelColor });
  await local;
}
...
export function createSarsaparillaFeature(moduleId: string): ModuleFeature {
  moduleSocket = `module.${moduleId}`;
